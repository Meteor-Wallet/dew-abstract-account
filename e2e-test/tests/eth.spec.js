...
test('onboard new ethereum user', async () => {
    const { factoryContractId, gasSponsor, nearJsonRpcProvider } =
        await globalSetup();

    // These values would come from the ethereum wallet
    ...
    upsertEnvVar('ETH_KEY', aliceWallet.privateKey);
});
...
test('sign transaction', async () => {
    ...
    // These values would come from the ethereum wallet
    const blockchainId = 'ethereum';
    const aliceWallet = new ethers.Wallet(process.env.ETH_KEY);
    const aliceBlockchainAddress = aliceWallet.address;
    ...
    const aliceSignTransactionSignature = await aliceWallet.signMessage(
