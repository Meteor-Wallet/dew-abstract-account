...
test('onboard new solana user', async () => {
    const { factoryContractId, gasSponsor, nearJsonRpcProvider } =
        await globalSetup();

    // These values would come from the Solana wallet
    const blockchainId = 'solana';
    const aliceKeypair = Keypair.generate();
    const aliceBlockchainAddress = aliceKeypair.publicKey.toBase58();
    ...
    const aliceCreateAccountSignature = nacl.sign.detached(
        new TextEncoder().encode(aliceMessageForCreateAccount),
    ...
    await gasSponsor.callFunction({
        ...
        args: {
            blockchain_id: blockchainId,
            blockchain_address: aliceBlockchainAddress,
            signature: bs58.encode(aliceCreateAccountSignature),
    ...
    upsertEnvVar('SOLANA_KEY', bs58.encode(aliceKeypair.secretKey));
});
...
test('sign transaction', async () => {
    ...
    const aliceKeypair = Keypair.fromSecretKey(
        bs58.decode(process.env.SOLANA_KEY)
    );
    const aliceBlockchainAddress = aliceKeypair.publicKey.toBase58();
    ...
    const aliceSignTransactionSignature = nacl.sign.detached(
        new TextEncoder().encode(aliceMessageForSignTransaction),
        aliceKeypair.secretKey
    );
        ...
        actions: [
            ...
            nearAPI.transactions.functionCall(
                'sign_transaction',
                {
                    ...
                    blockchain_address: aliceBlockchainAddress,
                    transaction,
                    signature: bs58.encode(aliceSignTransactionSignature),
                },
                200n * 10n ** 12n,
                0n
            ),
